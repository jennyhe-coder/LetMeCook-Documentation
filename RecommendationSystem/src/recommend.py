...
def try_update_missing_embedding(recipe_id):
    ...
    if row.empty:
        raise ValueError(f"❌ Recipe ID '{recipe_id}' not found in cleaned data")
...
def recommend_by_id(target_id, top_k=5):
    sync_embeddings()
...
def recommend_for_user(fav_ids, hist_ids, fav_weight=2.0, hist_weight=1.0, top_k=5):
    sync_embeddings()
